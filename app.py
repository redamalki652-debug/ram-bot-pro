...
st.set_page_config(page_title="RAM Bot v2.1", page_icon="🤖", layout="centered")
...
# تهيئة الـ session_state مرة وحدة
...
def translate_to_english(text):
    text = text.lower().replace("ولد ليا فيديو ديال", "").replace("ولد ليا صورة ديال", "").strip()
    dict = {"قط": "cat running, cute", "كلب": "dog running", "بحر": "ocean waves"}
...
def generate_video_replicate(prompt):
    ...
    try:
        with st.spinner("كنولد الفيديو 20 ثانية... ⚡"):
            output = replicate.run("wan-video/wan-2.2-a14b", input={
                "prompt": eng_prompt, "duration": 5, "aspect_ratio": "1:1", "fps": 16
            })
...
# خلي غير chat_input واحد باش ما يتبدلش النص
...
if audio and audio["bytes"] and not st.session_state.last_input:
    with st.spinner("كنسمعك..."):
        transcription = client.audio.transcriptions.create(
            file=("audio.wav", audio["bytes"]), model="whisper-large-v3", response_format="text", language="ar"
        )
...
if st.session_state.last_input:
    ...
    st.session_state.last_input = "" # مسح باش ما يعاودش
    ...
    if "فيديو" in user_input:
        with st.chat_message("assistant"):
            ...
            if isinstance(result, bytes):
                ...
                st.session_state.messages.append({"role": "assistant", "content": "تم", "video": result, "source": source, "audio": audio})
            else:
                st.error(result)
    ...
    # صورة
        ...
        with st.chat_message("assistant"):
            ...
            if result:
                ...
                st.audio(audio, format="audio/mp3")
                st.session_state.messages.append({"role": "assistant", "content": result, "image": "ai", "audio": audio})
            else:
    ...
    # دردشة عادية
        ...
        with st.chat_message("assistant"):
            chat_completion = client.chat.completions.create(
                messages=[{"role": "system", "content": "نتا RAM Bot. المطور رضا مالكي. كتهضر بالدارجة المغربية باختصار."},

...
if VISION_MODEL:
    ...
    st.warning("⚠️ ما كاينش Vision Model متاح")

with st.expander("📋 الموديلات المتاحة عندك"):
    st.code("\n".join(ALL_MODELS) if ALL_MODELS else "فارغ")
...
def call_groq_text(messages):
    ...
    try:
        chat_completion = client.chat.completions.create(
            messages=messages, model=TEXT_MODEL, temperature=0.7, max_tokens=MAX_TOKENS
        )
...
def call_groq_vision(user_question, image_base64, mime_type):
    ...
    try:
        messages_with_image = [{"role": "user","content": [{"type": "text", "text": user_question},{"type": "image_url", "image_url": {"url": f"data:{mime_type};base64,{image_base64}"}}]}]
        chat_completion = client.chat.completions.create(
            messages=messages_with_image, model=VISION_MODEL, temperature=0.7, max_tokens=MAX_TOKENS
        )
        return chat_completion.choices[0].message.content, None
    except Exception as e:
        return None, str(e)
    ...
    with st.spinner("كنسمعك..."):
        ...
        if user_text:
            ...
            if st.session_state.uploaded_image is not None:
                ...
                system_prompt = {"role": "system", "content": "نتا RAM Bot v4.8. المطور ديالك رضا مالكي. جاوب بالعربية المغربية باختصار."}
                response, error = call_groq_text([system_prompt] + st.session_state.messages)
                if error: st.error(f"🚨 خطأ Text: {error}")
                else:
                    audio_response = text_to_speech(response, detected_lang)
                    with st.chat_message("assistant"):
                        st.markdown(response)
...
if prompt_text_only:
    ...
    if any(word in prompt_text_only for word in ["ولد ليا", "generate", "draw"]):
        ...
        st.session_state.messages.append({"role": "user", "content": prompt_text_only})
        with st.chat_message("assistant"):
            system_prompt = {"role": "system", "content": "نتا RAM Bot v4.8. جاوب بالعربية المغربية باختصار."}
            response, error = call_groq_text([system_prompt] + st.session_state.messages)
            if error: st.error(f"🚨 خطأ Text: {error}")
            else:
                audio_response = text_to_speech(response, detected_lang)
